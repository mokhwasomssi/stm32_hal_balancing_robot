...
void read_gyro(I2C_HandleTypeDef* hi2c, mpu6050* __my_mpu6050, unit unit_you_want)
{
	...
	{
	case raw_data : //바로 저장
		__my_mpu6050->gy_x = (int16_t)(data_to_read[0] << 8 | data_to_read[1]) + 38;
		__my_mpu6050->gy_y = (int16_t)(data_to_read[2] << 8 | data_to_read[3]) + 178; 
		__my_mpu6050->gy_z = (int16_t)(data_to_read[4] << 8 | data_to_read[5]) + 346;
		break;
...
void read_accel(I2C_HandleTypeDef* hi2c, mpu6050* __my_mpu6050, unit unit_you_want)
{
	...
	{
	case raw_data : //바로 저장
		__my_mpu6050->ac_x = data_to_read[0] << 8 | data_to_read[1] - 944;
		__my_mpu6050->ac_y = data_to_read[2] << 8 | data_to_read[3] + 353;
		__my_mpu6050->ac_z = data_to_read[4] << 8 | data_to_read[5] + 1219;
		break;
